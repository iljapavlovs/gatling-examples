package lv.iljapavlovs.gatling.tutorials.advanced
...
class Advanced extends Simulation {
  ...
  object Browse {
    ...
    val browse: ChainBuilder = repeat(4, "i") { // Note how we force the counter name so we can reuse it
      exec(
        http("Page ${i}")
          .get("/computers?p=${i}")
      ).pause(1)
  ...
  object Edit {
    ...
    val edit: ChainBuilder = tryMax(2) { // let's try at max 2 times
      exec(
        http("Form")
          .get("/computers/new")
      ).pause(1)
